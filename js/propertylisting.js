...
{
    t.executeSql('DROP TABLE IF EXISTS propertyListing');
    t.executeSql('create table if not exists propertyListing(id INTEGER PRIMARY KEY AUTOINCREMENT,propertyid INTEGER, summary TEXT, imagecount integer,formvars TEXT,status TEXT, lastmod DATE,submitteddate DATE,imagesuploadeddate DATE)');
...
PropertyListing.prototype.getAll = function (start, callback)
{
    ...
    this.db.transaction(
		function (t)
		{
		    t.executeSql('select id,propertyid, summary,IFNULL(imagecount,0),formvars,status, lastmod,submitteddate,imagesuploadeddate from propertyListing order by propertyid asc', [],
				function (t, results)
				{
...
PropertyListing.prototype.getEntry = function (id, callback)
{
    ...
    this.db.transaction(
		function (t)
		{
		    t.executeSql('select id,propertyid, summary, imagecount,formvars,status, lastmod,submitteddate,imagesuploadeddate from propertyListing where id = ?', [id],
				function (t, results)
				{
...
PropertyListing.prototype.applyChanges = function (propertydata,callback)
{
    ...
    this.db.transaction(
        function (t)
        {
            var sql = "insert or replace into propertyListing ( id,propertyid, summary, imagecount,formvars,status, lastmod,submitteddate,imagesuploadeddate) values(?,?,?,?,?,?,?,?,?)";
            var params = [propertydata.id, propertydata.propertyid, propertydata.summary, propertydata.imagecount, propertydata.formvars, propertydata.status, propertydata.lastmod, propertydata.submitteddate, propertydata.imagesuploadeddate];
...
PropertyListing.prototype.savePropertyChanges = function (data, callback)
{
    ...
    this.db.transaction(
        function (t)
        {
            t.executeSql('insert or replace into propertyListing ( id, propertyid,summary, formvars,status, lastmod) values(?,?,?,?,?,?)', [data.id,data.id, data.summary, data.formvars, data.status, new Date().getTime()],
            function ()
            {
                callback();
            }, this.dbErrorHandler);
